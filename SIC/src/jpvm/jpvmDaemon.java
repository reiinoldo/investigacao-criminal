	...
	private boolean doExec() {
		try {
			...
			try {
				...
				perror("CreateTask, \"" + jpe + " sending " + "to client "
						+ client.toString());
			}
			return false;
		}
...
public class jpvmDaemon {
	...
	public static void main(String args[]) {
		try {
			...
			while (true) {
				...
				if (debug_on) {
					log("new request type=" + request + ",\"" + reqName
	...
	private static void RegisterChild(jpvmTaskId client, jpvmBuffer req) {
		...
		if (regNum < 0 || regNum >= maxCreateOrders) {
			perror("RegisterChild, child registration number " + regNum
					+ "out of bounds");
			return;
		}
	...
	private static void SpawnTask(jpvmTaskId client, jpvmBuffer req) {
		...
		if (num == 0) {
			buf.pack(num);
			try {
				jpvm.pvm_send(buf, client, jpvmDaemonMessageTag.jpvmdSpawnTask);
			} catch (jpvmException jpe) {
				perror("SpawnTask, problem sending " + "to client "
						+ client.toString());
			}
			return;
		}
		...
		for (int i = 0; i < num; i++) {
			...
			if (target.tid.equals(jpvm.pvm_mytid())) {
				...
				try {
					...
					perror("SpawnTask, error scheduling " + "on host "
							+ target.tid.toString());
				}
			}
	...
	private static void CreatedTask(jpvmTaskId client, jpvmBuffer req) {
		...
		try {
			...
			perror("CreatedTask, bad report from " + client.toString());
		}
		...
		if (order == null) {
			perror("CreatedTask, order number " + orderNum + " is not valid");
			return;
		}
	...
	private static void CreateTask(jpvmTaskId client, jpvmBuffer req) {
		...
		spawnThread = new jpvmExecTaskThread(jpvm, client, args,
				createOrders[nextCreateOrder]);
		nextCreateOrder++;
		spawnThread.start();
	}
	...
	private static void Ping(jpvmTaskId client, jpvmBuffer req) {
		try {
			jpvm.pvm_send(req, client, jpvmDaemonMessageTag.jpvmdPingReply);
		} catch (jpvmException jpe) {
